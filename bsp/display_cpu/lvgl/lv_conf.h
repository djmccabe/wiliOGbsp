/**
 * @file lv_conf.h
 * LVGL configuration for the FreeWili OG display CPU. LVGL v9.2.2.
 *
 * Derived from LVGL's own lv_conf_template.h (MIT -- see
 * THIRD-PARTY-NOTICES.md). What is set FOR THIS BOARD, and why:
 *
 *   LV_COLOR_DEPTH 16   the ST7789 is RGB565. LVGL renders native-endian and
 *                       st7789_blit() byte-swaps to the panel's big-endian
 *                       order itself, so there is deliberately NO swap set
 *                       here -- setting one would double-swap.
 *   LV_MEM_SIZE 64K     of the RP2040's 264 KB, on top of the two static
 *                       partial-render buffers in fwog_lvgl.c.
 *   LV_USE_OS NONE      the display CPU runs no RTOS.
 *   LV_USE_LOG 0        DIAG() is this tree's diagnostic channel, and stdio
 *                       is USB CDC only -- never UART0, which is the link.
 *
 * To change any of it, copy this file and point LV_CONF_PATH at your copy;
 * nothing here is required by the port itself.
 */
